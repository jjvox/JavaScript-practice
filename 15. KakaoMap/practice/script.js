;(function () {
  ...
  const mapContainer = new kakao.maps.Map($map, { // 카카오 map가져오기 . 
    center: new kakao.maps.LatLng(defaultPos.lat, defaultPos.lng),
    level: 3
  })

  const createMarkerImage = () => {  // 마크 이미지와 사이즈를 세팅함
  ...
  const createMarker = (lat, lng) => {  // 마크 이미지를 생성해 준다. 
  ...
  const successGeolocation = (position) => { // 현재 내 위치를 보여줌.
  ...
  const errorGeolocation = (error) => {  // 현재 내 위치 정보를 찾을 수 없음. 
    if(error.code ===1) {
      alert('위치 정보를 허용해주세요')
